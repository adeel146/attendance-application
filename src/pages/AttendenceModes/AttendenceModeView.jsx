...
import { AttendenceModeHeader } from "../../constants/constants";
...
import { getAcademicSingleList } from "../../services/attendenceModeServices";
...
function AttendenceModeView() {
  ...
  const attendenceId = localStorage.getItem("attendenceModeId");

  const navigate = useNavigate();
  // const { data: staffList, isLoading } = useQuery({
  //   queryKey: ["AttendenceMode_List", { attendenceId }],
  //   queryFn: () => getAcademicSingleList(attendenceId),
  // });
  const { data: staffList, isLoading } = useQuery({
    queryKey: ["attendence_list", { attendenceId }],
    queryFn: () => getAcademicSingleList(attendenceId),

    keepPreviousData: true,
  });

  console.log(staffList?.data, "staffList");
  ...
  const handleUpdate = (props) => {
    ...
    navigate("/attendence-mode/update");
  };
  ...
  const AttendancePeriodOptions = [
    {
      name: "Period By Period",
      value: 1,
    },
    {
      name: "One-Time Attendance/Day",
      value: 2,
    },
    {
      name: "  2-Time Attendance/Day",
      value: 3,
    },
  ];

  const dataToSend = [{ ...staffList?.data }];
  const data = React.useMemo(() => dataToSend, [staffList]);

  const columns = React.useMemo(
    () => [
      {
        Header: "Attendence Mode",
        accessor: "applicationMode",
        Cell: (row) => {
          const attendeModeName = AttendanceModeOption.find((val) => {
            if (val.value == row?.row?.original?.applicationMode) {
              return val;
            }
          });
          return (
            <div>
              <h1>{attendeModeName?.name}</h1>
            </div>
          );
        },
      },
      {
        Header: "Attendence Period",
        accessor: "attendenceMode",
        Cell: (row) => {
          const attendePeriod = AttendancePeriodOptions.find((val) => {
            if (val.value == row?.row?.original?.attendenceMode) {
              return val;
            }
          });
          return (
            <div>
              <h1>{attendePeriod?.name}</h1>
            </div>
          );
        },
      },
    ...
    <>
      ...
      <div>
        ...
        <div className="p-8">
          ...
          <div
            onClick={() => navigate("/attendence-mode/add")}
...
export default AttendenceModeView;
