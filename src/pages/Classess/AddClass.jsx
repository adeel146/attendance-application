...
function AddClass() {
  ...
  const { mutate: update, isLoading: isUpdating } = useMutation({
    mutationFn: (data) => updateClass(data, params["*"]),
    onSuccess: async (data) => {
      if (data?.success) {
        toast.success(data?.message);
        navigate(-1);
  ...
  const handleformSubmit = (data) => {
    ...
    if (params["*"]) {
      ...
      createClass({ ...data, courses: courseIds });
    }
  };
