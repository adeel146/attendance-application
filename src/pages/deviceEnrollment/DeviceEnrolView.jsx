...
import { getDeviceList } from "../../services/DeviceEnrollmentServices";
...
function DeviceEnrolView() {
  ...
  const columns = React.useMemo(
    () => [
...

      {
        Header: "Device Type",
        Cell: (cell) => {
          const deviceTypeSet = deviceTypeList?.find((val) => {
            if (val?.value == cell?.row?.original?.deviceType) {
              return val;
            }
          });
  ...
  return (
    <>
      <div>
        <DetailBar
          total={`${deviceData?.total_records} Device Enrollment`}
...
export default DeviceEnrolView;
