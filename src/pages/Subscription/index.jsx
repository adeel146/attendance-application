...
function Subscriptions() {
  ...
  const {
    ...
    isLoading,
    isFetching: fetchingSubscriptionList,
  ...
  const handleUpdate = (props) => {
    ...
    navigate(`update-subscripton`, { state: props.row.original });
  };

  const data = React.useMemo(() => SubscriptionList?.data, [SubscriptionList]);
  const columns = React.useMemo(
    () => SubscriptionsHeaders(handleDelete, handleUpdate),
    []
  );
  ...
  return (
    <>
      ...
      <div>
        <DetailBar
          total={`${SubscriptionList?.total_records} ${singlevsplural(
            SubscriptionList?.total_records,
            "subscription"
          )}`}
        ...
        <div className="p-8">
          ...
          <div
            onClick={() => navigate("add-subscripton")}
          ...
          <div className="mt-2">
            <CustomTable
              ...
              totalPages={SubscriptionList?.total_pages}
              isLoading={isLoading || fetchingSubscriptionList}
              page_number={SubscriptionList?.page_number}
            />
          </div>
        </div>
      </div>
