...
function UpdateProfile() {
  ...
  const { mutate: updateCalenderData, isLoading } = useMutation({
    mutationFn: (data) => {
      return updateCalenders(id, data);
    },
    onSuccess: (res) => {
      if (res?.success) {
  ...
  useEffect(() => {
    if (profileData) {
      setValue("school_Id", profileData?.data?.school_Id, {
        shouldDirty: true,
        shouldValidate: true,
      });
    }
