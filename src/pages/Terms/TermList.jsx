...
function TermList() {
  ...
  const { data: termListData, isLoading } = useQuery({
    queryKey: ["termList", { pageNumber, schoolId, pageSize, searchVal }],
    queryFn: () =>
      getTermList({
        name: searchVal,
        page_size: pageSize,
  ...
  const { mutate: delTerm, isLoading: loadingDelete } = useMutation({
    mutationFn: (id) => deleteTermList(id),
    onSuccess: async (data) => {
      if (data?.success) {
        toast.success(data?.message);
        setisOpenDeleteModal(false);
        queryClient.invalidateQueries(["termList"]);
      } else {
  ...
  const handleUpdate = (props) => {
    navigate(`/term/update/${props?.row?.original?.id}`);
  };

  const data = React.useMemo(() => termListData?.data, [termListData]);
  ...
  return (
    <>
      <div>
        <DetailBar
          total={`${termListData?.total_records} Terms`}
          heading="Terms List"
          ...
          {isLoading ? (
            ...
            <div className="mt-2">
              <CustomTable
                ...
                totalPages={termListData?.total_pages}
                setpageSize={setpageSize}
                page_number={termListData?.page_number}
              />
            </div>
          )}
        </div>
