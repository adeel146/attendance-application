...
import { commonPageSize, ParentsHeader } from "../../constants/constants";
...
function ParentList() {
  ...
  const {
    ...
    queryKey: ["parentList", { pageNumber, schoolId, pageSize, searchVal }],
    queryFn: () =>
      getParentsList({
        name: searchVal,
        page_size: pageSize,
  ...
  const handleUpdate = (props) => {
    navigate(`update-parent/${props.row.original.id}`);
  };

  const data = React.useMemo(() => parentList?.data, [parentList]);
  const columns = React.useMemo(
    () => ParentsHeader(handleDelete, handleUpdate),
    []
  );
  ...
  return (
    <>
      ...
      <div>
        <DetailBar
          // total={`${parentList?.total_records} ${singlevsplural(
          //   parentList?.total_records,
          //   "parent"
          // )}`}
          total={`${parentList?.total_records} Parents`}
          heading={t("parents_users_list")}
        ...
        <div className="p-8">
          ...
          <div className="mt-2">
            <CustomTable
              ...
              totalPages={parentList?.total_pages}
              setpageSize={setpageSize}
              page_number={parentList?.page_number}
            />
          </div>
        </div>
      </div>
