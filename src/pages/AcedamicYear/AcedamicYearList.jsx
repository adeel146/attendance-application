...
function AcedamicYearList() {
  ...
  const { data: academicListData, isLoading } = useQuery({
    queryKey: ["academicList", { pageNumber, schoolId, pageSize, searchVal }],
    queryFn: () =>
      getAcademicList({
        name: searchVal,
        page_size: pageSize,
  ...
  const { mutate: delAcList, isLoading: loadingDelete } = useMutation({
    mutationFn: (id) => deleteAcedamicList(id),
    onSuccess: async (data) => {
      if (data?.success) {
        toast.success(data?.message);
        setisOpenDeleteModal(false);
        queryClient.invalidateQueries(["academicList"]);
      } else {
  ...
  const handledeleteYear = () => {
    delAcList(obj?.row?.original?.id);
  };

  const handleUpdate = (props) => {
    navigate(`/academic-year/edit/${props?.row?.original?.id}`);
  };

  const data = React.useMemo(() => academicListData?.data, [academicListData]);
  ...
  const columns = React.useMemo(
    () => [
      ...
      },
      {
        ...
        accessor: "startDate",
        Cell: (row) => (
          <div>{moment(row.row.original.startDate).format("l")}</div>
        ),
      },
      {
        Header: "End Date",
        accessor: "endDate",
        Cell: (row) => (
          <div>{moment(row.row.original.endDate).format("l")}</div>
        ),
      },
  ...
  return (
    <>
      <ConfirmationModal
        ...
        handleSubmit={handledeleteYear}
        text="Academic Year"
        loading={loadingDelete}
      />
      <div>
        <DetailBar
          total={`${academicListData?.total_records} Academic Year`}
          heading={t("acedamic_year")}
          ...
          {isLoading ? (
            ...
            <div className="mt-2">
              <CustomTable
                ...
                totalPages={academicListData?.total_pages}
                setpageSize={setpageSize}
                page_number={academicListData?.page_number}
              />
            </div>
          )}
        </div>
...
export default AcedamicYearList;
