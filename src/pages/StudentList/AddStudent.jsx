...
function AddStudent() {
  ...
  if (params["*"]) {
    const { data: studentobj } = useQuery({
      queryKey: ["studentData"],
      queryFn: () => getSingleStudent(params["*"]),
  ...
  const { mutate: update } = useMutation({
    mutationFn: (data) => updateStudent(data, params["*"]),
    onSuccess: async (data) => {
      if (data?.success) {
        toast.success(data?.message);
        navigate(-1);
  ...
  const handleformSubmit = (data) => {
    ...
    if (params["*"]) {
      update({ ...data, groups: groupIds, classes: classes });
    } else {
      createStudent({ ...data, groups: groupIds, image, classes: classes });
    }
  };
